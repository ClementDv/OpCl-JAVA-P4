...
public class DBConstants {
    ...
    public static final String GET_NEXT_PARKING_SPOT =
            "select min(PARKING_NUMBER) from parking where"
    ...
    public static final String UPDATE_TICKET =
            "update ticket set PRICE=?, OUT_TIME=?, IS_PAID=? where ID=?";
    public static final String GET_TICKET =
            "select t.PARKING_NUMBER, t.VEHICLE_REG_NUMBER, t.ID, t.PRICE, "
                    + "t.IN_TIME, t.OUT_TIME, t.IS_PAID, p.AVAILABLE, p.TYPE "
                    + "from ticket t,parking p "
                    + "where p.parking_number = t.parking_number "
                    + "and t.VEHICLE_REG_NUMBER=? "
                    + "order by t.ID desc limit 1";
    public static final String GET_RECURRENT_USER =
